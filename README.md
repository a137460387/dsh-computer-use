...
- **Physical confirm gate for irreversible actions** (default off) — with `irreversibleConfirm` enabled, irreversible hotkeys (currently `shift+delete`, permanent deletion) and danger-pattern `type_text` hits pause desktop control and wait for the user to physically press the takeover hotkey instead of riding the approval seam or hard-refusing; a `resume_actions` call counts as a denial, a generous timeout closes the wait without resuming anything, and every wait writes `confirm/requested` / `confirm/granted` / `confirm/denied` audit lines.
...
- id: computer-use
  config:
    ...
    # ── Irreversible-action confirm gate (default off) ──
    # irreversibleConfirm: false    # route irreversible hotkeys (shift+delete)
                                    # and danger-pattern type_text hits through
                                    # the physical confirm gate; enabling fails
                                    # loud on macOS or with an empty takeoverHotkey
    # confirmTimeoutMs: 28800000    # generous wait backstop (8 h); on expiry the
                                    # action closes denied and desktop control
                                    # STAYS paused — nothing resumes automatically
...
- **Vision calls route by purpose.** Each call purpose picks a cost tier — `analysisTier` (screenshot analysis and coordinate localization), `changeDetectionTier` (screen-change judgement), `verificationTier` (post-action effect verdicts) — and each tier rides one of the two routes: `flash` the change-detection route, `pro` the vision route. Defaults keep analysis on `pro` and both judgement calls on `flash`; moving a purpose to a different tier is an explicit deployment choice. The tier that produced a verification verdict is recorded as `modelTier` on its `verification/result` audit line, and the before frame the verdict compared against as `observationId`.
...
- **Confirm-gate semantics** (`irreversibleConfirm`, default off): an irreversible hit pauses desktop control under the `confirm` reason and waits for the physical takeover hotkey — it deliberately skips the approval seam, because a quota auto-grant would bypass any human and a never-approval session would swallow the wait. Only a `hotkey`-reason resume releases the action; `resume_actions` is judged a denial; the `confirmTimeoutMs` backstop closes the wait as a denial but keeps desktop control paused (no auto-resume, no retry — the model must call `resume_actions` itself to continue with other work). One wait may be pending at a time; a second irreversible hit is refused until it settles. For danger-pattern `type_text`, the release arms a single-use sidecar token that lets exactly that payload pass the sidecar's aligned backstop (all other danger payloads stay blocked). Waits are audited as `confirm/requested` / `confirm/granted` / `confirm/denied` (the paired `lifecycle/paused` line carries the `confirm` reason). Enabling fails loud at activation on macOS or with an empty `takeoverHotkey`, where the physical confirm signal cannot exist.
...
- **An irreversible action is stuck waiting for a physical confirm (`irreversibleConfirm` on).** That is the gate working: desktop control is paused under the `confirm` reason and the action will not run until someone presses the takeover hotkey (release) or the wait is abandoned (`resume_actions` or the turn cancelling denies it; the `confirmTimeoutMs` backstop eventually denies it too). Every path leaves desktop control paused until resumed — nothing resumes on its own. The `confirm/requested`/`confirm/denied` audit lines name the trigger and the closure reason.
...
- **The confirm gate shares the takeover hotkey.** A hotkey press cannot tell "confirm the pending irreversible action" from "resume my interrupted session" — pressing it while a confirm waits releases the action. The irreversible list folds the `del` alias into `delete`; the pre-existing high-risk list still matches canonical key names only (its `esc` vs `escape` alias gap is a separate, recorded item).
...
- **Irreversible actions wait for a physical confirm** when `irreversibleConfirm` is on: `shift+delete` (hotkey) and danger-pattern payloads (type_text) pause desktop control and block until the user presses the takeover hotkey (release) or the wait times out / is resumed via `resume_actions` (denial). The refusal messages say which one happened and that the action was NOT performed.
